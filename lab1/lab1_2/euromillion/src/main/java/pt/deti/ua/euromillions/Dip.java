package pt.deti.ua.euromillions;

import java.util.Arrays;
import java.util.Objects;

import pt.deti.ua.sets.BoundedSetOfNaturals;
...
public class Dip {
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if( ! Arrays.stream(arrayOfNumbers).allMatch( nr -> nr >= 1 && nr <= NUMBERS_RANGE_MAX) ){
            throw new IllegalArgumentException("numbers set are outside the expected range 1!");
        }
